<template>
  <div class="recommended home-container">
    <div class="home-container031 d-flex">
      <div class="container row me-4 py-4">
        <span class="home-text009 text-star my-1">More Recommended Destination</span>
        <span class="home-text010">
          <span>Book Domestic and International Holiday Packages</span>
          <br />
        </span>
      </div>
      <div class="row">
        <div class="d col-12 col-sm-4">
          <a href="#">
            <div class="home-container050">
              <div class="hover01 column">
                <figure>
                  <img
                    alt="image"
                    src="/external/new-baner-03-1400w.jpg"
                    class="home-image11"
                  />
                </figure>
              </div>

              <section class="home-container033" id="hide7">
                <div class="home-container034">
                  <div class="home-container035">
                    <div class="home-container036">
                      <span class="home-text013">BAGHDAD</span>
                    </div>
                    <div class="home-container037">
                      <div class="home-container038">
                        <img
                          alt="image"
                          src="/external/icon-01-200h.png"
                          class="home-image12"
                        />
                      </div>
                    </div>
                    <div class="home-container039">
                      <span class="home-text014">MASHHAD</span>
                    </div>
                  </div>
                  <div class="home-container040">
                    <button type="button" class="home-button04 button">
                      <span class="home-text015">Facial services&nbsp;</span>
                      <svg viewBox="0 0 1024 1024" class="home-icon26">
                        <path
                          d="M366 708l196-196-196-196 60-60 256 256-256 256z"
                        ></path>
                      </svg>
                    </button>
                    <button type="button" class="home-button05 button">
                      <span class="home-text016">Search flights</span>
                      <svg viewBox="0 0 1024 1024" class="home-icon28">
                        <path
                          d="M406 598q80 0 136-56t56-136-56-136-136-56-136 56-56 136 56 136 136 56zM662 598l212 212-64 64-212-212v-34l-12-12q-76 66-180 66-116 0-197-80t-81-196 81-197 197-81 196 81 80 197q0 42-20 95t-46 85l12 12h34z"
                        ></path>
                      </svg>
                    </button>
                  </div>
                </div>
              </section>
            </div>
          </a>
        </div>

        <div class="d col-12 col-sm-4">
          <a href="#">
            <div class="home-container050">
              <div class="hover01 column">
                <figure>
                  <img
                    alt="image"
                    src="/external/new-baner-01-1400w.jpg"
                    class="home-image11"
                  />
                </figure>
              </div>

              <section class="home-container042" id="hide7">
                <div class="home-container043">
                  <div class="home-container044">
                    <div class="home-container045">
                      <span class="home-text017">BAGHDAD</span>
                    </div>
                    <div class="home-container046">
                      <div class="home-container047">
                        <img
                          alt="image"
                          src="/external/icon-01-200h.png"
                          class="home-image14"
                        />
                      </div>
                    </div>
                    <div class="home-container048">
                      <span class="home-text018">BEIRUT</span>
                    </div>
                  </div>
                  <div class="home-container049">
                    <button type="button" class="home-button06 button">
                      <span class="home-text019">Facial services&nbsp;</span>
                      <svg viewBox="0 0 1024 1024" class="home-icon30">
                        <path
                          d="M366 708l196-196-196-196 60-60 256 256-256 256z"
                        ></path>
                      </svg>
                    </button>
                    <button type="button" class="home-button07 button">
                      <span class="home-text020">Search flights</span>
                      <svg viewBox="0 0 1024 1024" class="home-icon32">
                        <path
                          d="M406 598q80 0 136-56t56-136-56-136-136-56-136 56-56 136 56 136 136 56zM662 598l212 212-64 64-212-212v-34l-12-12q-76 66-180 66-116 0-197-80t-81-196 81-197 197-81 196 81 80 197q0 42-20 95t-46 85l12 12h34z"
                        ></path>
                      </svg>
                    </button>
                  </div>
                </div>
              </section>
            </div>
          </a>
        </div>

        <div class="d col-12 col-sm-4">
          <a href="#">
            <div class="home-container050">
              <div class="hover01 column">
                <figure>
                  <img
                    alt="image"
                    src="/external/new-baner-02-1400w.jpg"
                    class="home-image11"
                  />
                </figure>
              </div>

              <section class="home-container051" id="hide7">
                <div class="home-container052">
                  <div class="home-container053">
                    <div class="home-container054">
                      <span class="home-text021">BAGHDAD</span>
                    </div>
                    <div class="home-container055">
                      <div class="home-container056">
                        <img
                          alt="image"
                          src="/external/icon-01-200h.png"
                          class="home-image16"
                        />
                      </div>
                    </div>
                    <div class="home-container057">
                      <span class="home-text022">DUBAI</span>
                    </div>
                  </div>
                  <div class="home-container058">
                    <button type="button" class="home-button08 button">
                      <span class="home-text023">Facial services&nbsp;</span>
                      <svg viewBox="0 0 1024 1024" class="home-icon34">
                        <path
                          d="M366 708l196-196-196-196 60-60 256 256-256 256z"
                        ></path>
                      </svg>
                    </button>
                    <button type="button" class="home-button09 button">
                      <span class="home-text024">Search flights</span>
                      <svg viewBox="0 0 1024 1024" class="home-icon36">
                        <path
                          d="M406 598q80 0 136-56t56-136-56-136-136-56-136 56-56 136 56 136 136 56zM662 598l212 212-64 64-212-212v-34l-12-12q-76 66-180 66-116 0-197-80t-81-196 81-197 197-81 196 81 80 197q0 42-20 95t-46 85l12 12h34z"
                        ></path>
                      </svg>
                    </button>
                  </div>
                </div>
              </section>
            </div>
          </a>
        </div>
      </div>
    </div>
  </div>
</template>